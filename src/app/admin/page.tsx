...
// ============================================================================
// 1. TIPE DATA LENGKAP (FULL TYPE DEFINITIONS)
// ============================================================================
...
export default function AdminPage() {
  ...
  // --- NOTIFIKASI SYSTEM (CUSTOM TOAST) ---
  ...
  const fetchAllData = async () => {
    ...
    try {
        ...
        showNotify("Gagal koneksi ke database. Periksa internet.", "error");
    } finally {
        setRefreshing(false);
    }
  };
  ...
  const handleUpdateMainLogo = async () => {
     if (!mainLogoFile) return showNotify("Pilih file gambar logo terlebih dahulu!", "error");
     setLoading(true);
     ...
     if (url) {
         ...
         if (error) {
             showNotify("Gagal menyimpan ke database: " + error.message, "error");
         } else {
             setMainLogoFile(null);
             showNotify("✅ Logo Website Berhasil Diupdate! Refresh halaman depan.", "success");
             fetchAllData();
         }
     }
     setLoading(false);
  ...
  const deleteItem = async (table: string, id: number) => {
    if(confirm("⚠️ Hapus data ini permanen? Tindakan tidak bisa dibatalkan.")) {
        ...
        if (error) showNotify("Gagal menghapus: " + error.message, "error");
        else {
  ...
  if (!session) return (
    <div className="min-h-dvh bg-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="bg-white w-full max-w-sm p-8 rounded-3xl shadow-2xl border border-slate-800 relative overflow-hidden">
        ...
        <p className="text-center text-xs text-slate-300 mt-8">System v15.1 (Full Power)</p>
      </div>
    </div>
  );
  ...
  return (
    ...
    <main className="ml-0 md:ml-72 flex-1 p-4 sm:p-6 md:p-10 pb-[calc(6rem+env(safe-area-inset-bottom))] md:pb-10 transition-all">
        ...
        {/* ================================================================================== */}
        {/* TAB 4: CMS (LANDING PAGE) - FULLY FEATURED */}
        {/* ================================================================================== */}
        {activeTab === "cms" && (
                        ...
                        <div className="p-5 flex-1 overflow-y-auto space-y-2 bg-slate-50/30">
                            {rundown.map((r, index) => (
                                <div key={r.id} className="flex items-center p-3 border border-slate-200 rounded-xl bg-white hover:shadow-sm transition-all group">
                                    <div className="mr-3 text-xs font-bold text-slate-300 w-4">{index + 1}.</div>
